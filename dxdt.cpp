...
double x_init = 1., y_init = 0., Vx_init = 0., Vy_init = 1. , theta_init = 0., w_init = 0.; 
double x0[] = { x_init, y_init, Vx_init, Vy_init, theta_init, w_init };
double t0 = 0., t1 = 45., dt = 0.0001;
...
Vector dx_dt( const double t, const Vector x)
{
    ...
    output[3] = - kappa*x[1] / pow(r, exponent+1.);
    output[4] = x[5];
    output[5] = - (3./pow(r,5)) * (x[0]*sin(x[4]) - x[1]*cos(x[4])) * (x[0]*cos(x[4]) + x[1]*sin(x[4]));
