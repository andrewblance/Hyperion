...
// initialising vector, and initial values of coordinates 
double x_init = a * (1 + eccentricity), Vx_init = 0., 
       y_init = 0., Vy_init = init_Vel * sqrt( GMs*(1.-eccentricity) / (a * (1.+eccentricity)) ),
       z_init = 0.01, Vz_init = 0.;
double x0[] = { x_init, Vx_init, y_init, Vy_init, z_init, Vz_init };
double t0 = 0., t1 = 9., dt = 0.0001;

const size_t n_dim = 6;
Vector x( x0, n_dim );

// Chaotic Tumbling of Hyperion (n_dim=6)
